...
<body>
<h3>Crear tabla de costes desde archivo</h3>
     <?php
          ...
          if( isset($_FILES['uploaded']['name']) ){
               ...
               if($_POST['operation']=="create_table"){ 
                    if(!isset($_POST['table'])||$_POST['table']==""){ 
                         ...
                         $table = "ws_cost_" . $_POST['table'];
                         //print "I will create the table first!<br>\n";
                         //print "The new table name is " . $table . "<br>\n";
                         $resultado = mysql_query("create table " . $table . " like ws_def_costs;") or die("No he podido CREAR la tabla! " . mysql_error());
                         MoveAndInsert($table);
                    }
               }
     ...
     <form enctype="multipart/form-data" action="costupload.php" method="POST">
          <table>
               <tr>
                    ...
                    <td>
                         <input type="text" name="table" id="table">(El sistema pondrá el prefijo "ws_cost_" automáticamente)
                    </td>
               </tr>
               <tr>
                    <td>
...
function MoveAndInsert($table){
     ...
     if(move_uploaded_file($_FILES['uploaded']['tmp_name'], $target)) 
     {
          ...
          $sqldatetime = "LOAD DATA LOCAL INFILE '" . $target . "' INTO TABLE $table FIELDS TERMINATED BY '$separator' (areacode, description, @var1) SET cost = replace(@var1, ',', '.');";
